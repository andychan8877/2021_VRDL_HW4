...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        # read image
        print(path)
        imgname, img_lq, img_gt_1 = get_image_pair(args, path)  # image to HWC-BGR, float32
        img_gt_1 = None
        print(img_gt_1)
        # print('RRRRRRRRRRRRRRRRRRRRRRRRRRRRR')
        ...
        if img_gt_1 is not None:
            img_gt_1 = (img_gt_1 * 255.0).round().astype(np.uint8)  # float32 to uint8
            img_gt_1 = img_gt_1[:h_old * args.scale, :w_old * args.scale, ...]  # crop gt
            img_gt_1 = np.squeeze(img_gt_1)

            psnr = util.calculate_psnr(output, img_gt_1, border=border)
            ssim = util.calculate_ssim(output, img_gt_1, border=border)
            test_results['psnr'].append(psnr)
            test_results['ssim'].append(ssim)
            if img_gt_1.ndim == 3:  # RGB image
                output_y = util.bgr2ycbcr(output.astype(np.float32) / 255.) * 255.
                img_gt_y = util.bgr2ycbcr(img_gt_1.astype(np.float32) / 255.) * 255.
            ...
            if args.task in ['jpeg_car']:
                psnr_b = util.calculate_psnrb(output, img_gt_1, border=border)
